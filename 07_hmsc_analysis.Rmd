...
pred_species <- constructGradient(m, 
             ...
             as.data.frame() %>%
             mutate(origin=c("domestic","feral")) %>%
...
pred_species %>%
      ...
      mutate(diff_grey.red = `domestic` - `feral`) %>%
...
pred_species %>%
      ...
      left_join(genome_metadata, by=join_by(genome==genome)) %>%
      mutate(genome= factor(genome, levels=genome_tree$tip.label)) %>% 
ggplot(aes(x=forcats::fct_rev(genome), y=diff_grey.red, color=phylum)) + 
  geom_point(size=4) + 
...
```{r hmsc_origin_significant_plot1, warning=FALSE, comments="", message=FALSE, fig.height=10, fig.width=8, fig.fullwidth=TRUE}
